...
public class WqpException extends Exception {
	...
	private final WqpException previous;         // the exception which was caught
	...
	public String traceBack(final String sep) {
		int level = 0;
		WqpException e = this;
		final StringBuilder text = new StringBuilder(line("WQPGatewayException Trace: Calling sequence (top to bottom)"));
		while (e != null) {
			level++;
			text.append(sep);
